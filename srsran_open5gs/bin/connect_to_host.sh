...
# Read remote host IP from hosts file
remote_ip=$(grep -w "$new_connection" /etc/hosts | awk '{print $1}')


# Exit when the remote host is unknown
if [ -z "$remote_ip" ]; then
...
# Extract the existing connection node
...
# Confirm replacement of an existing connection
